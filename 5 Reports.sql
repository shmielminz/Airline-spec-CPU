...
select BookedPeopleOnFlight = count(*), FlightNumber, DepartureAirport, TimeDeparting, ArrivalCountry
from flight 
group by FlightNumber, DepartureAirport, TimeDeparting, ArrivalCountry

    --2) Who isn't checked in for flights departing in the next week, in order to send them reminders to check in.
select *
from Flight f
where f.DateDeparting between getdate() and dateadd(day,7,getdate())

    --3) How many flights are departing per day, and num of passengers we have on those flights.
select NumOfFlights = count(*), f.DateDeparting, NumOfPassengers = count(f.PassengerLastName)
from Flight f
group by DateDeparting
