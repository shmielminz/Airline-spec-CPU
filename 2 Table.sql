-- SM Great work 98% see comments below and resubmit
use AirlineDB
go 
...
-- SM on all char add constraint that it cant be less
...
-- SM Use 1 column for date and time, it would help for constraint that departure can't be after arrival
...
-- SM this should be date of travel not todays date
...
-- SM should be as char()/varchar() with constraint that needs to be numeric and can't start with 0, so there shouldn't be 0 in reports.
...
-- SM sorry for making this to complicated but some of the getdate() should be issue date and some date of travel
